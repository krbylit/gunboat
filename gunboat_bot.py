...
#   (working?)need to figure out way to wait on both player commands,
#   maybe while loop with list of commands,
#   checking on length
#       maybe implement dictionary for complete turn history, may make 3x
#       maneuver tracking easiest
# need to figure out how to have bot send/receive DMs for private move
# commands, bot end of turn response should be
...
@bot.command(
    help="Challenge the channel members to a game of Gunboat. The first to "
    "accept with '$accept' will become your "
    "opponent",
...
async def accept(ctx):
    ...
    for player in game[
        "players"
    ]:  # for getting opposing player's name, look into better implementation
        if player != user_name:
            opponent = player
    game["players"][f"{user_name}"] = Frigate(user_name)
...
@bot.command(
    help="Command your crew to fire cannons. Deals 1 damage normally, "
    "takes 1 ammo. Can be dodged if the opponent "
    "maneuvers during "
    "the same turn.",
    brief="Fire cannons!",
    name="fire",
...
async def fire(ctx):
    ...
    await ctx.channel.send(
        f"{user_name} accepted {game['players']['challenger'].name}'s " f"challenge!"
    )
    await ctx.channel.send(rf"")


@bot.command(
    help="Command your crew to perform evasive maneuvers. Dodges cannon fire "
    "if fired upon during the same turn. "
    "Maneuvering 3 turns in a row puts you on their broadside, though, "
    "and you're susceptible to a critical hit.",
    brief="Evade!",
    name="man",
    # optional shorter command call, prefix w/ $ in chat to execute
)
async def maneuver(ctx):
    ...
    await ctx.channel.send(rf"""""")  # MAY NEED TO GET RID OF ``` CODE FORMATTING
...
async def turn_end(ctx):  # need to add message feedback for game states
    ...
    turn = game["turn"]
    player_1 = list(game["turn_log"][f"turn_{turn}"].values())[
        0
    ]  # listify values of current turn to index into
    # player
    ...
    game["turn"] += 1
    await ctx.channel.send(
        f"{user_name} accepted {game['players']['challenger'].name}'s challenge!"
    )
